...
router.get("/auth", (req, res) => {
  const redirectUri = encodeURIComponent("http://localhost:4000/api/linkedin/callback");
  const clientId = process.env.LINKEDIN_CLIENT_ID;
  const scope = encodeURIComponent("r_liteprofile r_emailaddress");
  const state = "xyz" + Date.now();

  const authUrl = `https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id=${process.env.LINKEDIN_CLIENT_ID}&redirect_uri=${encodeURIComponent(redirectUri)}&scope=r_liteprofile%20r_emailaddress`;
